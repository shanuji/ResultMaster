...
class _WorkbookDashboardScreenState extends State<WorkbookDashboardScreen> {
  ...
  @override
  void initState() { super.initState(); _loadWorkbookData(); }
  ...
  Future<void> _loadWorkbookData() async {
    ...
    if (_terms.isNotEmpty && (_activeTermId == null || !_terms.any((t) => t.id == _activeTermId))) _activeTermId = _terms.first.id;
  ...
  Widget build(BuildContext context) {
    ...
    if (_terms.isEmpty) return Scaffold(appBar: AppBar(title: const Text('Result Dashboard')), body: const Center(child: Text('No terms configured for this workbook.')));
          ...
          actions: [
            Padding(
              ...
              child: DropdownButton<int>(
                ...
                items: _terms.map((term) => DropdownMenuItem(value: term.id, child: Text(term.name))).toList(),
                onChanged: (newTermId) { if (newTermId != null) setState(() => _activeTermId = newTermId); },
              ),
            ),
          ],
          bottom: TabBar(isScrollable: true, tabs: [..._subjects.map((s) => Tab(text: s.name)), const Tab(text: 'Term Summary'), const Tab(text: 'Final Result')]),
        ),
        body: TabBarView(
          children: [
            ..._subjects.map((subject) => SubjectMarksTab(subjects: [subject], students: _students, allStudents: _students)),
