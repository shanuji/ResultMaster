...
class _WorkbookDashboardScreenState extends State<WorkbookDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      length: 4, // FIXED: Added new Student Tab
      child: Scaffold(
        body: Column(
          children: [
            WavyHeader(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade50, elevation: 0), onPressed: _importExcel, icon: const Icon(Icons.file_upload, size: 18, color: Colors.green), label: const Text('Upload Excel', style: TextStyle(color: Colors.green))),
                                const SizedBox(width: 8),
                                ElevatedButton.icon(style: ElevatedButton.styleFrom(elevation: 0), onPressed: () async { int nextRoll = 1; while (_students.any((s) => s.rollNo.trim() == nextRoll.toString())) nextRoll++; await DatabaseHelper.instance.insertLiveStudent(widget.workbookId, nextRoll.toString(), ""); _loadData(); }, icon: const Icon(Icons.add, size: 18), label: const Text("Add Student")),
                              ],
                            ),
                          ),
                          const Divider(height: 1),
                          Expanded(
                            child: SingleChildScrollView(scrollDirection: Axis.vertical, child: DataTable(
                              columnSpacing: 24, horizontalMargin: 16, headingTextStyle: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black87),
