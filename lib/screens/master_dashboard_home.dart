...
// 👇 ADDED NEW ARCHITECTURE IMPORTS HERE
...
class _MasterDashboardHomeState extends State<MasterDashboardHome> {
  ...
  Future<void> _refreshWorkbooks() async {
    ...
    try {
      // 👇 FETCHING DATA VIA NEW SQLITE REPOSITORY
  ...
  // 👇 REPLACED ALERT DIALOG WITH WIZARD PAGE NAVIGATION
  void _openNewWorkbookWizard() {
    Navigator.push(
      context,
      MaterialPageRoute(
      ...
      builder: (context) => AlertDialog( 
        ...
        actions: [ 
          ...
          TextButton(onPressed: () async { 
            // 👇 UPDATED DELETE TO USE REPOSITORY
            await _repository.deleteWorkbook(id); 
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          ...
          Expanded(
            child: _isLoading 
              ...
              : _isWorkbooksTab 
                ? _workbooks.isEmpty 
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.folder_open, size: 64, color: Colors.grey), const SizedBox(height: 16), const Text('No workbooks created yet.', style: TextStyle(fontSize: 16, color: Colors.grey)), const SizedBox(height: 16), ElevatedButton.icon(onPressed: _openNewWorkbookWizard, icon: const Icon(Icons.add), label: const Text('Create Workbook'))]))
                  : ListView.builder(
                      itemCount: _workbooks.length, padding: const EdgeInsets.symmetric(horizontal: 16),
                      itemBuilder: (context, index) {
                        final item = _workbooks[index];
                        // Extracting 'name' based on the new SQLite database columns
                        final String displayTitle = item['name'] ?? 'Unnamed Workbook';
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            trailing: Container(decoration: BoxDecoration(border: Border.all(color: Colors.red.shade100, width: 2), borderRadius: BorderRadius.circular(8)), child: IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent, size: 20), onPressed: () => _deleteWorkbookConfirm(item['id'], displayTitle))),
                            // 👇 FIXED: Removed workbookTitle argument to prevent the crash
                            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => WorkbookDashboardScreen(workbookId: item['id']))).then((_) => _refreshWorkbooks()),
                          ),
                        );
                      }
                    )
      ...
      floatingActionButton: _isWorkbooksTab ? FloatingActionButton(onPressed: _openNewWorkbookWizard, backgroundColor: Theme.of(context).colorScheme.primary, foregroundColor: Colors.white, child: const Icon(Icons.add)) : null,
    );
  }
}
