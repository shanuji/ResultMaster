...
class SqliteResultWorkbookRepository {
  ...
  Future<List<TermSetup>> getWorkbookTerms(int workbookId) async {
    ...
    return results.map((map) => TermSetup.fromMap(map)).toList();
  }
  ...
  Future<List<SubjectSetup>> getWorkbookSubjects(int workbookId) async {
    ...
    for (var row in rawData) {
      ...
      if (row['component_id'] != null) {
        subjectsMap[subId]!.components.add(SubjectComponent(
  ...
  Future<List<StudentRow>> getStudentsWithFullData(int workbookId) async {
    ...
    Map<int, StudentRow> studentMap = {};

    for (var s in studentsRaw) {
      int sId = s['id'] as int;
      studentMap[sId] = StudentRow(
        id: sId,
