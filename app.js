...
function renderMarks() {
  ...
  state.students.forEach((student, index) => {
    ...
    tr.innerHTML = `<td>${index + 1}</td><td>${student.roll_no}</td><td>${student.name}</td>` +
      state.subjects.map(subject => `<td><input type="number" min="0" max="${subject.max_marks || subject.maximum_marks || state.settings.max_marks}" step="0.01" value="${markValue(student.id, subject.id)}" data-student="${student.id}" data-subject="${subject.id}"></td>`).join('') +
...
async function onMarkChange(event) {
  ...
  if (marks < 0 || marks > Number(subject?.max_marks || subject?.maximum_marks || state.settings.max_marks || 0)) {
...
function renderPrintStudentPicker() {
  const picker = $('#print-student');
  if (!picker) return;
  picker.innerHTML = state.students.map(student => `<option value="${student.id}">${student.roll_no} - ${student.name}</option>`).join('');
}
...
function renderSettings() {
  ...
  $('#school-name').value = state.settings.school_name || '';
  $('#school-address').value = state.settings.school_address || '';
  $('#school-logo').value = state.settings.school_logo || '';
  $('#page-size').value = state.settings.page_size || 'A4';
  $('#page-margin-mm').value = state.settings.page_margin_mm || 12;
  $('#header-text').value = state.settings.header_text || 'Report Card';
  $('#footer-text').value = state.settings.footer_text || 'Generated by ResultMaster';
  $('#teacher-name').value = state.settings.teacher_name || '';
  $('#principal-name').value = state.settings.principal_name || '';
  ...
  $('#subject-rules').innerHTML = state.subjects.map(subject => `<div class="subject-rule"><strong>${subject.name}</strong><label><input type="checkbox" data-subject="${subject.id}" data-field="include_in_pass_fail" ${isIncluded(subject, 'include_in_pass_fail') ? 'checked' : ''}> Include in Pass/Fail</label><label><input type="checkbox" data-subject="${subject.id}" data-field="include_in_percentage" ${isIncluded(subject, 'include_in_percentage') ? 'checked' : ''}> Include in Percentage</label></div>`).join('');
...
async function saveSettings() {
  ...
  state.settings.school_name = $('#school-name').value;
  state.settings.school_address = $('#school-address').value;
  state.settings.school_logo = $('#school-logo').value;
  state.settings.page_size = $('#page-size').value;
  state.settings.page_margin_mm = Number($('#page-margin-mm').value || 12);
  state.settings.header_text = $('#header-text').value;
  state.settings.footer_text = $('#footer-text').value;
  state.settings.teacher_name = $('#teacher-name').value;
  state.settings.principal_name = $('#principal-name').value;
...
$('#print-class').addEventListener('click', () => window.open('/api/print?scope=class', '_blank'));
$('#print-student-button').addEventListener('click', () => window.open('/api/print?scope=student&student_id=' + encodeURIComponent($('#print-student').value), '_blank'));
loadWorkbook();
