...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with db() as con:
            if path == "/api/marks":
                subject = con.execute("SELECT max_marks AS maximum_marks FROM subjects WHERE id = ?", (data["subject_id"],)).fetchone()
                if subject is None:
                ...
                for subject in data.get("subjects", []):
                    con.execute(
                        "UPDATE subjects SET include_in_pass_fail = ?, include_in_percentage = ? WHERE id = ?",
                        (1 if subject.get("include_in_pass_fail", subject.get("include_pass_fail")) else 0, 1 if subject.get("include_in_percentage", subject.get("include_percentage")) else 0, subject["id"]),
                    )
            else:
                self.send_error(404)
                return
